...
> **文档版本**：v2.1（整合 Onboarding 引导页与游客登录/授权页）
> **编写日期**：2026-07-21
> **作者**：产品通（基于设计概念稿扩展）
> **状态**：评审稿
...
- 作为**首次打开的用户**，我希望一眼看懂产品价值与用法，以便决定是否继续。
- 作为**犹豫是否授权的用户**，我希望先以游客身份逛逛，以便零负担感受产品。
- 作为**游客**，我希望提前知道哪些功能受限，以便不被突然拦截。
...
│   ├── F-M5-01 情绪色谱（四色）
│   └── F-M5-02 动效与声效规范
├── 🔴 M9 新用户引导 Onboarding（首启前置）
│   ├── F-M9-01 欢迎与价值主张（引导页 1）
│   └── F-M9-02 核心功能与场景引导（引导页 2）
├── 🔴 M10 授权与游客模式 Guest Access（首启前置）
│   ├── F-M10-01 游客模式快速入口
│   ├── F-M10-02 游客权限引导与限制说明
│   └── F-M10-03 微信授权升级（游客 → 账号）
└── ⚪ 未来规划（暂不在本期）
    ├── 多城市扩展与方言微气候
    ├── 无障碍增强（语音盲盒）
    └── 主题皮肤（季节限定）
...
### 3.5 🔴 新增页面：Onboarding 引导页（2 页）与游客登录/授权页

> 优先级说明：Onboarding 与游客模式是核心体验的"前置闸门"。产品主张"反刻意、低门槛"，若首启即强制微信授权或缺乏引导，会直接劝退散步型低频用户。故将 M9 / M10 列为 🔴 核心（MVP 必须），但允许"游客态"作为降级默认，授权按需触发。

#### F-M9-01 引导页 1 · 欢迎与价值主张
- **页面目的**：首次启动即传递"产品是什么、对我有什么用"，降低认知门槛，帮助用户 10 秒内决策是否继续。
- **目标用户**：首次打开小程序、对产品无先验认知的用户（覆盖画像 A / B / C 全量新客）。
- **页面流程**：小程序冷启 → 检测本地 `onboarding_done` → 缺失则全屏展示引导页 1 → 用户【开始体验】进入引导页 2 / 【跳过】进入授权判断页。
- **交互说明**：全屏布局，顶部品牌名（Logo 占位）+ 一句话价值主张（≤ 20 字），中部三个场景微缩插画（顺路拾取 / 转角释放 / 周末回顾），底部【开始体验】主 CTA（暖意·橙），右上角弱化【跳过】。状态清单：

| 状态 | 触发 | UI 表现 | 用户操作 |
|------|------|---------|----------|
| 默认 | 首启 | 品牌 + 价值主张 + 场景插画 | 浏览 / 点击开始 |
| 跳过 | 点右上角"跳过" | 直接进入授权判断页 | 去游客或登录 |
| 已完成 | 本地标记存在 | 不再展示 | 直接进入罗盘 |

- **边界条件**：引导仅首次展示；`onboarding_done` 存本地，换设备 / 清缓存后可能再展示（可接受）。【跳过】不强制授权，直接进入游客模式（见 M10）。
- **异常处理**：插画资源加载失败 → 占位渐变背景 + 文字保留，不阻断流程。
- **验收标准**：引导页首屏 ≤ 1.5s；价值主张文案 ≤ 20 字；跳过入口常驻可见。

#### F-M9-02 引导页 2 · 核心功能与场景引导
- **页面目的**：用最少步骤讲清"怎么用"，降低首次行动的心理成本，让用户在散步前不迷茫。
- **目标用户**：已认可价值主张、准备开始使用的新用户。
- **页面流程**：引导页 1【开始体验】→ 单页内左右滑动切换 2–3 屏（情绪罗盘 / 近场拾取 / 转角释放）→ 末屏【进入街角】→ 授权判断页。
- **交互说明**：每屏含插画 + 一句话说明 + 底部进度点（●○○）；支持右滑返回上一屏（首屏禁用右滑）；滑动与按钮均可推进，统一动效 `cubic-bezier(0.25,1,0.5,1)` 250ms；末屏按钮文案"进入街角"。状态清单：

| 状态 | 触发 | UI 表现 | 用户操作 |
|------|------|---------|----------|
| 第 1 屏 | 进入 | 情绪罗盘说明 + 插画 | 左滑 / 点下一步 |
| 第 2 屏 | 左滑 | 近场拾取说明 | 左滑 / 点下一步 |
| 第 3 屏 | 左滑 | 转角释放说明 | 点【进入街角】 |
| 末屏 | 到达 | 【进入街角】按钮 | 跳转授权判断 |

- **边界条件**：进度点指示当前屏，点击进度点不可跳屏（防误触）；滑动与按钮推进动效一致。
- **异常处理**：滑动中断（来电 / 切后台）→ 保留当前屏，回前台恢复，不跳屏。
- **验收标准**：引导总时长 ≤ 15s；末屏按钮文案"进入街角"；完成即写 `onboarding_done`。

#### F-M10 游客登录 / 授权判断页
- **页面目的**：在首启与正式使用之间提供"零授权快速体验"与"微信登录解锁"的双入口，用最低门槛承接新用户，并按需引导授权。
- **目标用户**：完成 / 跳过引导、尚未决定授权的用户；以及游客态下触发受限功能、希望升级的用户。
- **页面流程**：引导页 2【进入街角】或【跳过】→ 授权判断页 → ①【以游客身份体验】→ 情绪罗盘（游客态）；②【微信登录】→ `wx.login` → 情绪罗盘（账号态）；游客受限功能触发 → 回弹本页登录引导。
- **交互说明**：
  - 游客入口为主 CTA 按钮（暖意·橙底色，零授权），微信登录为次级文字按钮；页面含"游客模式"标签与权限对照折叠卡（✅ 可：浏览罗盘 / 拾取 / 接收温柔；⛔ 限：发布盲盒 / 查看温柔足迹 / 生成 AIGC 图卷）。
  - 游客态下罗盘页"+"发布入口置灰，点击即弹登录引导（不静默失败）。
  - 微信登录仅取 openid，不强制手机号 / 位置（位置按需触发）。
  - 状态清单：

| 状态 | 触发 | UI 表现 | 用户操作 |
|------|------|---------|----------|
| 引导说明 | 点游客入口 | 权限对照卡（✅ 可 / ⛔ 限） | 知道了 / 先体验 |
| 受限触发 | 游客点发布 | 轻提示"登录后留温柔" + 登录入口 | 去登录 / 取消 |

- **边界条件**：
  - 游客身份由小程序本地匿名标识承载（无需 `wx.login`），不展示昵称 / 头像。
  - 游客数据不持久化：足迹 / 图卷为空；登录后不产生"匿名数据合并"（设计决策，避免归属混乱）。
- **异常处理**：取消授权 → 停留游客态，不阻断浏览；网络失败 → 提示重试，不丢游客进度；游客误触发布 → 弹"登录后解锁"引导，不报错崩溃。
- **验收标准**：游客入口 1 次点击进入；微信登录 1 次点击；受限功能触发必弹登录引导；授权失败不崩、可回退游客态。
...
```
街角情绪盲盒小程序
├── 新用户引导与授权 Onboarding & Access（首启前置）
│   ├── 欢迎与价值主张 Welcome（引导页 1）
│   ├── 核心功能与场景引导 Guide（引导页 2）
│   └── 授权判断 Auth（游客 / 微信登录）
...
### 5.4 页面跳转逻辑
- 入口：微信会话 / 扫码 / 分享卡片 → 若本地 `onboarding_done` 缺失 → 引导页 1（P0）→ 引导页 2（P0'）→ 授权判断页（P5）。
- 授权判断页【以游客身份体验】→ 情绪罗盘页（P1，游客态）。
- 授权判断页【微信登录】→ `wx.login` → 情绪罗盘页（P1，账号态）。
- P1 "+" → 情绪打包浮层（P3，模态，可返回）；游客态下该入口置灰并触发登录引导。
- P1 围栏触发 → 微光卡片（P2，叠加浮层）。
- P1 底部"足迹" → 温柔足迹页（P4）；游客态下为空态并引导登录。
- P1 / P4 受限功能触发 → 回弹授权判断页（P5）或登录引导浮层。
- 全局缓动 `cubic-bezier(0.25,1,0.5,1)`。

### 5.5 新增页面线框图（引导页 / 授权判断页）
...
- **身份双态（新增 M10）**：① 游客态 = 本地匿名标识（localStorage 生成，不上报 openid），仅用于本次会话的浏览 / 拾取 / 接收，数据不持久化；② 账号态 = `wx.login` 获取 openid 后单向绑定 `code`，解锁发布 / 足迹 / 图卷。游客转账号为"新建"而非"合并"（避免匿名数据归属混乱）。
...
| 接收成功 | "已收到这份温柔" | 正向反馈 |
| 引导价值主张 | "把 15 分钟步行圈，变成有温度的情绪空间" | 首屏 ≤ 20 字 |
| 游客入口 | 以游客身份体验 | 零授权主按钮 |
| 游客受限提示 | "登录后可以把温柔留在这里" | 受限触发轻提示 |
| 授权判断标题 | 准备好了吗？先不登录也能逛逛 | 轻量、低压力 |
...
| **M1 MVP 核心** | 第 3–6 周 | M1 罗盘 + M2 近场解锁 + M3 发布 + M7 偏移 + M8 定位 + M6 风控 + **M9 引导 + M10 游客/授权** | 拾取 / 释放主链路可跑 + 首启引导与低门槛入口 |
...
- **引导页 Onboarding**：首启前置的全屏引导（价值主张 + 功能场景），仅首次展示。
- **游客模式 Guest Mode**：零授权快速体验态，可浏览 / 拾取 / 接收，受限发布 / 足迹 / 图卷。
- **授权判断页 Auth Page**：引导完成后的入口页，提供"游客体验 / 微信登录"双入口。
...
| v2.1 | 2026-07-24 | 整合 Onboarding 引导页（2 页）与游客登录/授权页：补充功能树 M9/M10、3.5 详细描述（页面目的/目标用户/页面流程/交互说明/异常处理）、信息架构/动线/线框图/跳转逻辑、身份双态数据说明、文案与术语、修订记录 |
